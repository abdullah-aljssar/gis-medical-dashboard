...
import { useDashboardStore, Facility } from '@/lib/store';
...
export function TimeMachine() {
  ...
  async function loadSnapshot(isoDate: string) {
    ...
    try {
      ...
      if (!data.snapshot) {
        setError('لا توجد لقطة محفوظة قبل هذا الوقت بعد');
        return;
      }

      setFacilities(data.snapshot.facilities.map((f: Facility) => ({ ...f })));
      setAmbulances(data.snapshot.ambulances);
  ...
  function returnToLive() {
    ...
    fetch('/api/facilities').then((r) => r.json()).then((d) => d.facilities && setFacilities(d.facilities));
    fetch('/api/ambulances').then((r) => r.json()).then((d) => d.ambulances && setAmbulances(d.ambulances));
  }
  ...
  return (
    ...
    >
      <span style={{ color: 'var(--ink-dim)', whiteSpace: 'nowrap' }}>⏱ الآلة الزمنية</span>
      ...
      {/* مؤشر بكرة الشريط: صفّ من الشُّرَط يمتلئ كتقدّم البكرة بدل شريط تمرير عادي */}
      <div style={{ flex: 1, display: 'flex', gap: 2, alignItems: 'center', height: 16, minWidth: 120 }}>
