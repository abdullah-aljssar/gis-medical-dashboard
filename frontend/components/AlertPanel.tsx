...
const KIND_COLOR: Record<string, string> = {
  MANUAL_DISPATCH: 'var(--accent)',
  EMERGENCY_CALL: 'var(--status-amber)',
  OCCUPANCY_CRITICAL: 'var(--status-red)',
};
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <span
          style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--status-red)' }}
          className="pulse-red"
        />
        <h2 style={{ fontSize: 13, fontWeight: 700 }}>تنبيهات فورية</h2>
        {alerts.length > 0 && (
          <span className="data-mono" style={{ fontSize: 10, color: 'var(--ink-faint)', marginInlineStart: 'auto' }}>
            {alerts.length}
          </span>
        )}
      </div>
      ...
      <div style={{ overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 8 }}>
        {alerts.length === 0 && (
          <p style={{ fontSize: 12, color: 'var(--ink-faint)' }}>
            لا توجد تنبيهات حالياً — كل المنشآت ضمن الحدود الطبيعية.
          </p>
        )}
        {alerts.map((a, i) => (
          <div
            ...
            style={{
              borderInlineStart: `2px solid ${KIND_COLOR[a.kind] || 'var(--status-red)'}`,
